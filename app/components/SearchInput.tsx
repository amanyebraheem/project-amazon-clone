...
// import { fetchData } from '../lib'; // تأكد أن هذا موجود ويعمل
// import Link from 'next/link';
// import { CiSearch } from 'react-icons/ci';
// import { Product } from '@/app/lib/Type'; // تأكد أن النوع Product معرف بشكل صحيح
...
//         console.log('Fetched products:', data?.products); // للتأكد من وصول البيانات
...
import { HiOutlineSearch } from 'react-icons/hi';
import { MdOutlineClose } from 'react-icons/md';
import { fetchData } from '../lib'; // تأكد أن fetchData معرفة بشكل صحيح
import Link from 'next/link';
import { CiSearch } from 'react-icons/ci';
import { Product } from '@/app/lib/Type'; // تأكد أن النوع معرف
        ...
        <div className="absolute left-0 top-12 w-full max-h-96 bg-white rounded-md shadow-md z-50 border border-[#fa8900]/50 overflow-y-auto">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => (
              <Link
                key={product.id}
                href={`/product/${product.id}`}  // تعديل الرابط ليناسب app router
                onClick={() => setSearchQuery('')}
                className="flex items-center gap-x-2 text-base font-medium hover:bg-gray-100 px-3 py-1.5"
              >
                <CiSearch className="text-lg" />
...
//   categories: string[]; // او النوع المناسب بناءً على اللي بتجيبه من fetchData
// }
